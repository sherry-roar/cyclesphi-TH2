...
DeviceInfo blender_device_info(BL::Preferences &b_preferences, BL::Scene &b_scene, bool background)
{
...
#ifdef WITH_CLIENT_RAAS
  DeviceInfo device = Device::available_devices(DEVICE_MASK_CLIENT).front();
#else
  ...
  if (BlenderSession::device_override != DEVICE_MASK_ALL) {
    ...
    if (compute_device != COMPUTE_DEVICE_CPU) {
      ...
      else if (compute_device == COMPUTE_DEVICE_CLIENT) {
        return Device::available_devices(DEVICE_MASK_CLIENT).front();
      }
