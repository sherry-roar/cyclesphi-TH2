...
class CyclesRenderSettings(bpy.types.PropertyGroup):
    ...
    volume_bounds_bounces: IntProperty(
        name="Volume Bounds Bounces",
        description="Maximum number of volumetric scattering events",
        min=0, max=1024,
        default=1024,
    )
...
class CyclesPreferences(bpy.types.AddonPreferences):
    ...
    def get_device_types(self, context):
        import _cycles
        has_cuda, has_optix, has_hip, has_metal, has_oneapi, has_client = _cycles.get_device_types()
        ...
        if has_client:
            list.append(('CLIENT', "Client", "Use Blender Client", 7))
    ...
    def update_device_entries(self, device_list):
        for device in device_list:
            if not device[1] in {'CUDA', 'OPTIX', 'CPU', 'HIP', 'METAL', 'ONEAPI', 'CLIENT'}:
                continue
    ...
    def get_devices_for_type(self, compute_device_type):
        ...
        if len(devices) and compute_device_type != 'CPU' and compute_device_type != 'CLIENT':
    ...
    def refresh_devices(self):
        ...
        for device_type in ('CUDA', 'OPTIX', 'HIP', 'METAL', 'ONEAPI', 'CLIENT'):
