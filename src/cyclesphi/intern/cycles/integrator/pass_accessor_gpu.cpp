                                               ...
                                               const Destination &destination) const
{
  ...
  const int offset = (buffer_params.offset + buffer_params.window_x +
...
#ifdef BLENDER_CLIENT
  if (destination.d_pixels_uchar_yuv) {
    const DeviceKernel kernel_uchar_float = static_cast<DeviceKernel>(kernel + 3);

    DeviceKernelArguments args(&kfilm_convert,
                               &destination.d_pixels_uchar_yuv,
                               &render_buffers->buffer.device_pointer,
                               &work_size,
                               &buffer_params.window_width,
                               &offset,
