...
#    include "../../../client/cycles/kernel_cuda.h"
#    include "device/cuda/device_impl.h"
#    include "device/cuda/util.h"
#  endif
#  if defined(WITH_CLIENT_OPTIX)
#    include "../../../client/cycles/kernel_optix.h"
#    include "device/optix/device_impl.h"
#    include "device/optix/util.h"
#  endif
#  if defined(WITH_CLIENT_HIP)
#    include "../../../client/cycles/kernel_hip.h"
...
#ifdef BLENDER_CLIENT
// void PathTraceWorkGPU::copy(const PathTraceWorkGPU &ptg)
//{
//   integrator_state_gpu_ = ptg.integrator_state_gpu_;
//   integrator_queue_counter_ = ptg.integrator_queue_counter_;
//
//   integrator_shader_sort_counter_ = ptg.integrator_shader_sort_counter_;
//   integrator_shader_raytrace_sort_counter_ = ptg.integrator_shader_raytrace_sort_counter_;
//   integrator_shader_sort_prefix_sum_ = ptg.integrator_shader_sort_prefix_sum_;
//
//   integrator_next_main_path_index_ = ptg.integrator_next_main_path_index_;
//   integrator_next_shadow_path_index_ = ptg.integrator_next_shadow_path_index_;
//
//   queued_paths_ = ptg.queued_paths_;
//   num_queued_paths_ = ptg.num_queued_paths_;
//
//   work_tiles_ = ptg.work_tiles_;
//   //*device_scene_ = *ptg.device_scene_;
// };

#  if defined(WITH_CLIENT_CUDA) || defined(WITH_CLIENT_OPTIX)
void PathTraceWorkGPU::queue_set_stream(CUstream s)
{
  ((CUDADeviceQueue *)queue_.get())->set_stream(s);
}

CUDADevice &PathTraceWorkGPU::cuda_device() const
{
  return *(CUDADevice *)device_;
};
#  endif

#  ifdef WITH_CLIENT_HIP
void PathTraceWorkGPU::queue_set_stream(hipStream_t s)
{
  ((HIPDeviceQueue *)queue_.get())->set_stream(s);
}

HIPDevice &PathTraceWorkGPU::hip_device() const
{
  return *(HIPDevice *)device_;
};
#  endif

void PathTraceWorkGPU::kernels_use_stat(bool us)
{
#  ifdef WITH_CUDA_STAT
  cuda_device().kernels.set_use_stat(us);
#  endif

#  ifdef WITH_HIP_STAT
  hip_device().kernels.set_use_stat(us);
#  endif
}

void PathTraceWorkGPU::set_buffer(const KernelWorkTile &wtile,
                                  ...
                                  int h)
{
  ...
  // effective_buffer_params_.full_x = 0; //wtile.x;
  // effective_buffer_params_.full_y = 0;  // wtile.y;
  // effective_buffer_params_.full_width = w;
  // effective_buffer_params_.full_height = h;
...
#  ifdef WITH_CLIENT_CUDA_GPU_TILES
  effective_buffer_params_.gpu_id = wtile.gpu_id;
  effective_buffer_params_.num_gpus = wtile.num_gpus;
#  endif

  effective_buffer_params_.w2 = w;
  effective_buffer_params_.h2 = h;
