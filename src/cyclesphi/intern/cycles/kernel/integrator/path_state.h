...
#if defined(WITH_CPU_STAT) || defined(__KERNEL_CUDA_STAT__) || defined(__KERNEL_HIP_STAT__)
    kernel_data_fetch(transparent_bounces, 0);
#endif
...
#  if defined(WITH_CPU_STAT) || defined(__KERNEL_CUDA_STAT__) || defined(__KERNEL_HIP_STAT__)
    kernel_data_fetch(volume_bounces, 0);
#  endif
  }
  else
#endif
...
#if defined(WITH_CPU_STAT) || defined(__KERNEL_CUDA_STAT__) || defined(__KERNEL_HIP_STAT__)
        kernel_data_fetch(diffuse_bounces, 0);
#endif
      }
      else {
...
#if defined(WITH_CPU_STAT) || defined(__KERNEL_CUDA_STAT__) || defined(__KERNEL_HIP_STAT__)
        kernel_data_fetch(glossy_bounces, 0);
#endif
      }
    }
    else {
...
#if defined(WITH_CPU_STAT) || defined(__KERNEL_CUDA_STAT__) || defined(__KERNEL_HIP_STAT__)
      kernel_data_fetch(transmission_bounces, 0);
#endif
    }
...
#ifdef __VOLUME__
ccl_device_inline bool path_state_volume_next(KernelGlobals kg, IntegratorState state)
{
  /* For volume bounding meshes we pass through without counting transparent
   * bounces, only sanity check in case self intersection gets us stuck. */
  uint32_t volume_bounds_bounce = INTEGRATOR_STATE(state, path, volume_bounds_bounce) + 1;
#  if defined(WITH_CPU_STAT) || defined(__KERNEL_CUDA_STAT__) || defined(__KERNEL_HIP_STAT__)
  kernel_data_fetch(volume_bounds_bounces, 0);
#  endif
  INTEGRATOR_STATE_WRITE(state, path, volume_bounds_bounce) = volume_bounds_bounce;
  if (volume_bounds_bounce > kernel_data.integrator.max_volume_bounds_bounce) {
    return false;
  }
