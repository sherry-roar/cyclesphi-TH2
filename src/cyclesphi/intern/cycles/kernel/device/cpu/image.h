...
ccl_device float half_to_float(half h)
{
  //float f;
...
ccl_device float4 half4_to_float4(half4 h)
{
  float4 f;
...
#if defined(WITH_CPU_STAT)
#  include <omp.h>

extern int kernel_cpu_image_enable_stat;

#  define DATA_CPU_IMAGE_ACCESS_ADD(index, counter, counter_mul) \
    { \
      if (kernel_cpu_image_enable_stat) { \
        int tid = omp_get_thread_num(); \
        unsigned long long int id = (unsigned long long int)((double)(index) * \
                                                             (double)(counter_mul)); \
        counter[tid][id] = counter[tid][id] + 1; \
      } \
    }

#elif defined(__KERNEL_CUDA_STAT__)
#  define DATA_CPU_IMAGE_ACCESS_ADD(index, counter, counter_mul) \
    { \
      unsigned long long int id = (unsigned long long int)((double)(index) * \
                                                           (double)(counter_mul)); \
      unsigned long long int *c = (unsigned long long int *)(counter); \
      atomicAdd(&c[id], 1ULL); \
    }

#elif defined(__KERNEL_HIP_STAT__)
#  define DATA_CPU_IMAGE_ACCESS_ADD(index, counter, counter_mul) \
    { \
      unsigned long long int id = (unsigned long long int)((double)(index) * \
                                                           (double)(counter_mul)); \
...
template<typename TexT, typename OutT = float4> struct TextureInterpolator {
  ...
  static ccl_always_inline OutT read(const TexT *data,
                                     ...
                                     double counter_mul)
  {
    DATA_CPU_IMAGE_ACCESS_ADD(
        (long long int)y * (long long int)width + (long long int)x, counter, counter_mul);

    return read(data[(long long int)y * (long long int)width + (long long int)x]);
  }
  ...
  static ccl_always_inline OutT read(const TexT *data,
                                     ...
                                     double counter_mul)
  {
    DATA_CPU_IMAGE_ACCESS_ADD((long long int)x + (long long int)y * (long long int)width +
                                  (long long int)z * (long long int)width * (long long int)height,
                              counter,
                              counter_mul);

    return read(data[(long long int)x + (long long int)y * (long long int)width +
                     (long long int)z * (long long int)width * (long long int)height]);
  }
  ...
  static ccl_always_inline OutT trilinear_lookup(
      const TexT *data,
      float tx,
      float ty,
      float tz,
      ...
      int height,
      int depth,
      OutT read(const TexT *, int, int, int, int, int, int, unsigned long long int **, double),
      unsigned long long int **counter,
      double counter_mul)
  {
    OutT r = (1.0f - tz) * (1.0f - ty) * (1.0f - tx) *
             read(data, ix, iy, iz, width, height, depth, counter, counter_mul);
    r += (1.0f - tz) * (1.0f - ty) * tx *
         read(data, nix, iy, iz, width, height, depth, counter, counter_mul);
    r += (1.0f - tz) * ty * (1.0f - tx) *
         read(data, ix, niy, iz, width, height, depth, counter, counter_mul);
    r += (1.0f - tz) * ty * tx *
         read(data, nix, niy, iz, width, height, depth, counter, counter_mul);

    r += tz * (1.0f - ty) * (1.0f - tx) *
         read(data, ix, iy, niz, width, height, depth, counter, counter_mul);
    r += tz * (1.0f - ty) * tx *
         read(data, nix, iy, niz, width, height, depth, counter, counter_mul);
    r += tz * ty * (1.0f - tx) *
         read(data, ix, niy, niz, width, height, depth, counter, counter_mul);
    r += tz * ty * tx *
         read(data, nix, niy, niz, width, height, depth, counter, counter_mul);
    return r;
  }

  /** Tricubic Interpolation */
  static ccl_always_inline OutT tricubic_lookup(
      const TexT *data,
      float tx,
      float ty,
      float tz,
      ...
      double counter_mul)
  {
     ...
     */
#define DATA(x, y, z) \
  (read(data, xc[x], yc[y], zc[z], width, height, depth, counter, counter_mul))
...
#undef DATA
  ...
  static ccl_always_inline OutT interp_closest(const TextureInfo &info, float x, float y)
  {
    ...
    return read((const TexT *)data, ix, iy, width, height, info.counter, info.counter_mul);
  }
  ...
  static ccl_always_inline OutT interp_linear(const TextureInfo &info, float x, float y)
  {
    ...
    return (1.0f - ty) * (1.0f - tx) *
               read_clip(data, ix, iy, width, height, info.counter, info.counter_mul) +
           (1.0f - ty) * tx *
               read_clip(data, nix, iy, width, height, info.counter, info.counter_mul) +
           ty * (1.0f - tx) *
               read_clip(data, ix, niy, width, height, info.counter, info.counter_mul) +
           ty * tx * read_clip(data, nix, niy, width, height, info.counter, info.counter_mul);
  }
  ...
  static ccl_always_inline OutT interp_cubic(const TextureInfo &info, float x, float y)
  {
     ...
     * Lets the compiler inline all the matrix multiplications.
     */
#define DATA(x, y) (read_clip(data, xc[x], yc[y], width, height, info.counter, info.counter_mul))
#define TERM(col) \
  (v[col] * \
...
#undef DATA
                                                  ...
                                                  float z)
  {
    ...
    const TexT *data = (const TexT *)info.data;
    return read(data, ix, iy, iz, width, height, depth, info.counter, info.counter_mul);
  }
      ...
      interp_3d_cubic(const TextureInfo &info, float x, float y, float z)
  {
    ...
    switch (info.extension) {
      ...
      case EXTENSION_CLIP: {
        ...
        return tricubic_lookup((const TexT *)info.data,
                               tx,
                               ty,
                               tz,
                               xc,
    ...
    return tricubic_lookup(
        data, tx, ty, tz, xc, yc, zc, width, height, depth, read, info.counter, info.counter_mul);
  }
