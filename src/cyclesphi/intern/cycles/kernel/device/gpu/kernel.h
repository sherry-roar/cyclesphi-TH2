                             ...
                             const int max_tile_work_size)
{
  ...
  if (x % tile->num_gpus != tile->gpu_id) {
    return;
  }
#endif
...
ccl_device_inline void kernel_gpu_film_convert_byte_write_yuv(ccl_global uchar4 *rgba,
                                                          const int rgba_offset,
                                                          //const int wx,
                                                          //const int wy,
                                                          const int x,
                                                          ...
                                                          const uchar4 uchar_pixel)
{
  unsigned char* dst_y = (unsigned char*)rgba;
  unsigned char* dst_u = (unsigned char*)rgba + rgba_stride * h;
  unsigned char* dst_v = (unsigned char*)rgba + rgba_stride * h + rgba_stride * h ;/// 4;
    
  unsigned char r = uchar_pixel.x;
  unsigned char g = uchar_pixel.y;
  unsigned char b = uchar_pixel.z;

  int index = rgba_offset + x + y * rgba_stride;

  // Y
  //int index_y = /*rgba_offset +*/ (x + wx) + (y + wy) * rgba_stride;
  dst_y[index] = ((66 * r + 129 * g + 25 * b) >> 8) + 16;

  // U
  //if (x % 2 == 0 && y % 2 == 0) {
      //int index_u = /*rgba_offset/2 + */ ((x + wx) / 2) + ((y + wy) / 2) * (rgba_stride / 2);
      dst_u[index] = ((-38 * r + -74 * g + 112 * b) >> 8) + 128;
  //}

  // V
  //if (x % 2 == 0 && y % 2 == 0) {
      //int index_v = /*rgba_offset/2 +*/ ((x + wx) / 2) + ((y + wy) / 2) * (rgba_stride / 2);
      dst_v[index] = ((112 * r + -94 * g + -18 * b) >> 8) + 128;
  //}
}            
...
\
    ccl_global float *pixel = pixels + \
                              (rgba_offset + x + y * rgba_stride) * kfilm_convert.pixel_stride; \
\
...
\
  ccl_gpu_kernel(GPU_KERNEL_BLOCK_NUM_THREADS, GPU_KERNEL_MAX_REGISTERS) \
      ccl_gpu_kernel_signature(film_convert_##variant##_byte_yuv, \
                               const KernelFilmConvert kfilm_convert, \
                               ccl_global uchar4 *rgba, \
                               ccl_global float *render_buffer, \
                               int num_pixels, \
...
\
    const uchar4 uchar_pixel = make_uchar4( \
        ...
        (uchar)(saturatef(float_linear2srgb(pixel[2])) * 255.0f), \
        (uchar)(saturatef(pixel[3]) * 255.0f)); \
    kernel_gpu_film_convert_byte_write_yuv( \
        rgba, rgba_offset, x, y, rgba_stride, rgba_height, uchar_pixel); \
  } \
  ccl_gpu_kernel_postfix
