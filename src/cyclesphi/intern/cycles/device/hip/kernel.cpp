...
#  ifdef WITH_HIP_STAT
  hipModule_t hipModuleStat = device->hipModuleStat;

  for (int i = 0; i < (int)DEVICE_KERNEL_NUM; i++) {
    HIPDeviceKernel &kernel = kernels_stat_[i];
    ...
    const std::string function_name = std::string("kernel_gpu_") +
                                      device_kernel_as_string((DeviceKernel)i);
    hip_device_assert(
        device, hipModuleGetFunction(&kernel.function, hipModuleStat, function_name.c_str()));
...
#  ifdef WITH_HIP_STAT
  if (use_stat)
    return kernels_stat_[(int)kernel];
#  endif
...
#  ifdef WITH_HIP_STAT
  if (use_stat)
    return kernels_stat_[(int)kernel].function != nullptr;
#  endif
