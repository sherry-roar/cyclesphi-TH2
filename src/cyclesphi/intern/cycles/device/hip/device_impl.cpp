...
#  ifdef BLENDER_CLIENT
  HIPMem *cmem = &hip_mem_map[mem.device_pointer];
#  else
...
#  ifdef BLENDER_CLIENT
    const HIPMem &cmem = hip_mem_map[mem.device_pointer];
#  else
...
#  ifdef BLENDER_CLIENT
    hip_mem_map.erase(hip_mem_map.find(mem.device_pointer));
#  else
...
#    ifdef WITH_HIP_STAT
  // hip_assert(hipModuleGetGlobal(&mem, &bytes, hipModuleStat, name));
  // hip_assert(hipMemcpyHtoD(mem, host, size));

  hip_assert(hipModuleGetGlobal(&mem, &bytes, hipModuleStat, "kernel_params"));
  assert(bytes == sizeof(KernelParamsHIP));
...
#  ifdef BLENDER_CLIENT
  const_copy_to(mem.name.c_str(), &mem.device_pointer, sizeof(mem.device_pointer));
#  else
...
void HIPDevice::tex_alloc(device_texture &mem)
{
  ...
  if (!mem.is_resident(this)) {
    thread_scoped_lock lock(hip_mem_map_mutex);
#  ifdef BLENDER_CLIENT
    cmem = &hip_mem_map[mem.device_pointer];
#  else
