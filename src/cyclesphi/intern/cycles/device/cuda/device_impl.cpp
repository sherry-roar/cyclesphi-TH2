...
#ifdef BLENDER_CLIENT
  CUDAMem *cmem = &cuda_mem_map[mem.device_pointer];
#else
...
#ifdef BLENDER_CLIENT
    const CUDAMem &cmem = cuda_mem_map[mem.device_pointer];
#else
...
#  ifdef BLENDER_CLIENT
    cuda_mem_map.erase(cuda_mem_map.find(mem.device_pointer));
#  endif
...
void CUDADevice::tex_alloc(device_texture &mem)
{
  ...
  if (!mem.is_resident(this)) {
    thread_scoped_lock lock(cuda_mem_map_mutex);
#ifdef BLENDER_CLIENT
    cmem = &cuda_mem_map[mem.device_pointer];
#else
