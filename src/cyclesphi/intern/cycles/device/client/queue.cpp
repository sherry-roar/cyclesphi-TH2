...
CLIENTDeviceQueue::CLIENTDeviceQueue(CLIENTDevice *device)
    : DeviceQueue(device), client_device_(device)//, cuda_stream_(nullptr)
{
  //const CUDAContextScope scope(cuda_device_);
  //cuda_device_assert(cuda_device_, cuStreamCreate(&cuda_stream_, CU_STREAM_NON_BLOCKING));
}

CLIENTDeviceQueue::~CLIENTDeviceQueue()
{
  //const CUDAContextScope scope(cuda_device_);
  //cuStreamDestroy(cuda_stream_);
}

int CLIENTDeviceQueue::num_concurrent_states(const size_t) const
{
...
void CLIENTDeviceQueue::init_execution()
{
...
//bool CLIENTDeviceQueue::kernel_available(DeviceKernel kernel) const
//{
//  //return cuda_device_->kernels.available(kernel);
//  return true;
//}

bool CLIENTDeviceQueue::enqueue(DeviceKernel kernel,
                     const int work_size,
                     DeviceKernelArguments const &args)
{
  ...
  //const CUDAContextScope scope(cuda_device_);
  //const CLIENTDeviceKernel &cuda_kernel = cuda_device_->kernels.get(kernel);
  ...
  //  case DEVICE_KERNEL_CONVERT_TO_BYTE:
  //  case DEVICE_KERNEL_CONVERT_TO_YUV:
...
bool CLIENTDeviceQueue::synchronize()
{
  //if (cuda_device_->have_error()) {
  //  return false;
  //}
...
void CLIENTDeviceQueue::zero_to_device(device_memory &mem)
{
...
void CLIENTDeviceQueue::copy_to_device(device_memory &mem)
{
  //assert(mem.type != MEM_GLOBAL && mem.type != MEM_TEXTURE);
...
void CLIENTDeviceQueue::copy_from_device(device_memory &mem)
{
  //assert(mem.type != MEM_GLOBAL && mem.type != MEM_TEXTURE);
