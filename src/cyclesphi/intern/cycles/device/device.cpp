...
vector<DeviceInfo> Device::client_devices;
...
#ifdef WITH_CLIENT
    case DEVICE_CLIENT:
      if (device_client_init())
        device = device_client_create(info, stats, profiler);
      break;
#endif
    default:
      break;
...
#ifdef WITH_CLIENT
  if (mask & DEVICE_MASK_CLIENT) {
    if (!(devices_initialized_mask & DEVICE_MASK_CLIENT)) {
      if (device_client_init()) {
        device_client_info(client_devices);
      }
      devices_initialized_mask |= DEVICE_MASK_CLIENT;
    }
    foreach (DeviceInfo &info, client_devices) {
      devices.push_back(info);
    }
  }
#endif
