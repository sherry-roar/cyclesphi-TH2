...
{
#ifndef WITH_CLIENT_FILE_MINISCENE
  ...
  uint *object_prim_count = dscene->object_prim_count.alloc(scene->objects.size());
  foreach (Object *ob, scene->objects) {
    ...
    if (Geometry *const geom = ob->geometry) {
      ...
      if (geom->geometry_type == Geometry::MESH) {
        prim_count = ((Mesh *const)geom)->num_triangles();
      }
    }
    uint obj_index = ob->get_device_index();
    object_prim_offset[obj_index] = prim_offset;
    object_prim_count[obj_index] = prim_count;
  }

  dscene->object_prim_offset.copy_to_device();
  dscene->object_prim_count.copy_to_device();
  dscene->object_prim_offset.clear_modified();
  dscene->object_prim_count.clear_modified();
}
...
void ObjectManager::device_free(Device *, DeviceScene *dscene, bool force_free)
{
  ...
  dscene->object_prim_count.free_if_need_realloc(force_free);
}
