...
static int outliner_all_level_exec(bContext *C, wmOperator *op)
{
  ...
  for (i = 1; i <= level; i++) {
    if (add == 1) {
      outliner_openclose_level(&soops->tree, 1, i, 1);
    }
    else {
      outliner_openclose_level(&soops->tree, 1, i, 0);
    }
  }
...
void OUTLINER_OT_show_all_level(wmOperatorType *ot)
{
  PropertyRNA *prop;

  /* identifiers */
  ot->name = "Show/Hide All Level";
  ot->idname = "OUTLINER_OT_show_all_level";
  ot->description = "Expand/collapse all entries by all level";

  /* callbacks */
  ot->exec = outliner_all_level_exec;
  ot->poll = ED_operator_outliner_active;

  /* no undo or registry, UI option */

  /* properties */
  prop = RNA_def_boolean(ot->srna, "open", 1, "Open", "Expand all entries all level deep");
  RNA_def_property_flag(prop, PROP_SKIP_SAVE);
}
