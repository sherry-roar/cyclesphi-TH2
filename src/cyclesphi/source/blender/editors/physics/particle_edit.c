...
static bool start_edit_poll(bContext *C)
{
  // if (PE_hair_poll(C)) {
  //  Scene *scene = CTX_data_scene(C);
  //  ParticleEditSettings *pset = PE_settings(scene);
...
static int start_edit_exec(bContext *C, wmOperator *UNUSED(op))
{
  Depsgraph *depsgraph = CTX_data_ensure_evaluated_depsgraph(C);
  Scene *scene = CTX_data_scene(C);
  Object *ob = CTX_data_active_object(C);
...
void PARTICLE_OT_start_edit(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Start Edit";
  ot->idname = "PARTICLE_OT_start_edit";
  ot->description = "Switch to edit";

  /* api callbacks */
  ot->exec = start_edit_exec;
  ot->poll = start_edit_poll;
...
static bool update_edit_poll(bContext *C)
{
  // if (PE_hair_poll(C)) {
  //  Scene *scene = CTX_data_scene(C);
  //  ParticleEditSettings *pset = PE_settings(scene);
...
static int update_edit_exec(bContext *C, wmOperator *UNUSED(op))
{
  Depsgraph *depsgraph = CTX_data_ensure_evaluated_depsgraph(C);
  Scene *scene = CTX_data_scene(C);
  Object *ob = CTX_data_active_object(C);
...
void PARTICLE_OT_update_edit(wmOperatorType *ot)
{
  /* identifiers */
  ot->name = "Update Edit";
  ot->idname = "PARTICLE_OT_update_edit";
  ot->description = "Update edit";

  /* api callbacks */
  ot->exec = update_edit_exec;
  ot->poll = update_edit_poll;
