...
static void rna_def_particle_settings(BlenderRNA *brna)
{
  ...
  prop = RNA_def_property(srna, "use_particle_state_instance", PROP_BOOLEAN, PROP_NONE);
  RNA_def_property_boolean_sdna(prop, NULL, "draw", PART_DRAW_HAIR_DUPLI);
  RNA_def_property_ui_text(prop, "Particle State", "Use particle state for position of object");
  RNA_def_property_update(prop, 0, "rna_Particle_redo");
