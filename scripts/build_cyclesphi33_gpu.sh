...
output=${ROOT_DIR}/install/cyclesphi33_gpu
src=${ROOT_DIR}/src
...
#-----------blender_client--------------
mkdir ${ROOT_DIR}/build/cyclesphi33_gpu
cd ${ROOT_DIR}/build/cyclesphi33_gpu
...
make_d="${make_d} -DWITH_CLIENT_UNIMEM=OFF"
make_d="${make_d} -DENABLE_INC_SAMPLES=ON"
make_d="${make_d} -DENABLE_STEP_SAMPLES=ON"
make_d="${make_d} -DENABLE_LOAD_BALANCE=ON"

make_d="${make_d} -DWITH_CLIENT_FILE=ON"  ## ON
make_d="${make_d} -DWITH_CLIENT_MPI_FILE=ON"  ## ON
make_d="${make_d} -DCLIENT_MPI_LOAD_BALANCING_SAMPLES=ON"
########
make_d="${make_d} -DWITH_CLIENT_MPI_FILE_1=ON" ## OFF
make_d="${make_d} -DWITH_CLIENT_MPI_SOCKET=OFF" ## OFF
...
cp ${ROOT_DIR}/build/cyclesphi33_gpu/cycles_cuda/*.cubin ${output}/lib/.
